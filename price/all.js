...
function renderData(arr){
    ...
    arr.forEach(function(item){
        ...
        <td>${item.上價}</td><td>${item.中價}</td><td>${item.下價}</td>
        <td>${item.平均價}</td><td>${item.交易量}</td></tr>`
    });
...
typeName.addEventListener("click",function(e){
    ...
    //為點擊到的按鈕增加active屬性
    let typeNameList =document.querySelectorAll("#typeName button");
    typeNameList.forEach(item=>{item.classList.remove('active')});
    ...
    selectList.addEventListener("change",function(e){
        let currentOpt= e.target.value;  //宣告currentOpt為點擊到的value值
            ...
            if(currentOpt==currentOpt){
                optData=arr.sort((a,b)=>{return b[currentOpt] - a[currentOpt]});
            }
            renderData(optData);
    });
}  
    ...
    upAndDown.addEventListener("click",function(e){
        ...
        function sortData(){
            if(e.target.dataset.sort=="up"){
                upDownData=arr.sort((a,b)=>{return a[value] - b[value]});
            }else if(e.target.dataset.sort=="down"){
                upDownData=arr.sort((a,b)=>{return b[value] - a[value]});
            }
        }
        if(value==value){
            sortData();
